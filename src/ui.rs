...
lazy_static::lazy_static! {
    ...
    static ref APP_ICON: String = format!(
        "data:image/png;base64,{}",
        STANDARD.encode(include_bytes!("../res/128x128.png"))
    );
}
...
pub fn start(args: &mut [String]) {
    ...
    #[cfg(all(target_os = "linux", feature = "inline"))]
    {
        ...
        let mut so_path = "/usr/share/duckdesk/libsciter-gtk.so".to_owned();
        for (prefix, dir) in [
            ("", "/usr"),
            ("", "/app"),
            (&app_dir, "/usr"),
        ...
        .iter()
        {
            let path = format!("{prefix}{dir}/share/duckdesk/libsciter-gtk.so");
