...
const ADDRESS_BOOK_ALIAS_LICENSE_PATH_KEYS: &[&str] = &[
    ...
    "address-book-alias-license-file",
    "address_book_alias_license_file",
    "license-path",
    "license_path",
    "license-file",
    "license_file",
...
fn update_heartbeat_alias(alias: String) {
    if Config::get_option(HEARTBEAT_ALIAS_OPTION) != alias {
        Config::set_option(HEARTBEAT_ALIAS_OPTION.to_owned(), alias);
    }
}

fn address_book_alias_license_path() -> String {
    // Keep the canonical key explicit and accept the underscore spelling for custom.json files
    // generated by tooling that normalizes JSON keys.
    let hard_settings = config::HARD_SETTINGS.read().unwrap();
    for &key in ADDRESS_BOOK_ALIAS_LICENSE_PATH_KEYS {
        if let Some(path) = hard_settings.get(key) {
            let path = path.trim();
            if !path.is_empty() {
    ...
    // Also allow the setting in custom.json's default-settings/override-settings sections.
    for &key in ADDRESS_BOOK_ALIAS_LICENSE_PATH_KEYS {
        let path = Config::get_option(key);
...
fn resolve_address_book_alias_license_path(configured_path: &str) -> PathBuf {
    ...
    #[cfg(windows)]
    {
        PathBuf::from("C:/").join(relative_path)
    }
    #[cfg(not(windows))]
    {
        Config::get_home().join(relative_path)
    }
}

fn parse_address_book_alias_license(contents: &str) -> Option<String> {
    contents.lines().find_map(|line| {
        let line = line.trim();
        let alias = line.strip_prefix('#')?.strip_suffix('#')?.trim();
...
async fn read_address_book_alias_license(configured_path: &str) -> Option<(PathBuf, String)> {
    let path = resolve_address_book_alias_license_path(configured_path);
    let contents = match tokio::fs::read_to_string(&path).await {
        Ok(contents) => contents,
        Err(err) => {
            log::debug!(
                "Failed to read address-book alias license file {:?}: {}",
                path,
                err
            );
            return None;
    ...
    let Some(alias) = parse_address_book_alias_license(&contents) else {
        log::warn!(
            "Address-book alias license file {:?} does not contain an alias in #alias# format",
            path
        );
        return None;
    };
...
async fn update_address_book_alias(
    ...
    let Some(alias) = take_heartbeat_alias(response) else {
        update_heartbeat_alias(String::new());
        state.pending_alias = None;
        return;
    };
    update_heartbeat_alias(alias.clone());
...
mod address_book_alias_tests {
    ...
    fn parses_alias_from_license_marker() {
        assert_eq!(
            parse_address_book_alias_license("ee2112d6a7822e08c126c4113913f9e1\n#zzx#\n"),
            Some("zzx".to_owned())
        );
    }
